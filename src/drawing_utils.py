...
# TODO: add the confidence parameter
def draw_label(I, l, color=(255, 0, 0), thickness=1, confidence=None):
    wh = np.array(I.shape[1::-1]).astype(float)
    tl = tuple((l.tl() * wh).astype(int).tolist())
    br = tuple((l.br() * wh).astype(int).tolist())

    if confidence is not None:
        cv2.putText(cv2.rectangle(I, tl, br, color, thickness=thickness), text=str(confidence), org=br,
...
def write2img(Img, label, strg, txt_color=(0, 0, 0), bg_color=(255, 255, 255), font_size=1, lp_confidence=None):
    ...
    if lp_confidence is not None:
        cv2.putText(Img, strg + "   " + str(lp_confidence), tpl(bl_corner), font, font_size, txt_color, 3)
    else:
